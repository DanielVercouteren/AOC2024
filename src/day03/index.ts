...
const parseInput = (rawInput: string) => [...rawInput.matchAll(/mul\((\d{1,3}),(\d{1,3})\)/g)]
const parseInputWithEnabledInstructions = (rawInput) => [...rawInput.matchAll(/(mul\((\d{1,3}),(\d{1,3})\))|(do\(\))|(don't\(\))/g)]
